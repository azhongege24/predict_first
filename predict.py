...
class POP_VA_para(QMainWindow, Ui_VA_para, Ui_MainWindow):
    ...
    def display_analysis_results(self):
        ...
        try:
            ...
            # 确保每页显示的分段数量至少为1
            segments_per_page_divisor = max(1, self.results_per_page // 2)
            
            # 计算总页数（每个分段占2行：频域图和时域图）
            if segments_per_page_divisor > 0:
                self.total_pages = (num_segments + segments_per_page_divisor - 1) // segments_per_page_divisor
            else:
                self.total_pages = 1
            ...
            # 计算当前页显示的分段范围
            segments_per_page = max(1, self.results_per_page // 2)
            start_idx = self.current_page * segments_per_page
            end_idx = min(start_idx + segments_per_page, num_segments)
            
            # 为当前页的分段创建子图
            num_to_show = end_idx - start_idx
            
            if num_to_show > 0:
                # 垂直排列显示当前页的分段，每个分段占2行
                for i in range(num_to_show):
                    result_idx = start_idx + i
                    result = results[result_idx]
                    
                    # 频域图（上方的子图）
                    ax_freq = self.figure.add_subplot(num_to_show * 2, 1, i * 2 + 1)
                    ax_freq.plot(result['frequency'], result['power_spectrum'], 'b-', linewidth=1.5)
                    ax_freq.set_xlabel('频率 (Hz)')
                    ax_freq.set_ylabel('功率谱密度 (g$^2$/Hz)')
                    
                    # 时域图（下方的子图）- 包含RMS值
                    ax_time = self.figure.add_subplot(num_to_show * 2, 1, i * 2 + 2)
                    
                    # 绘制时域信号（左侧y轴）
                    ax_time.plot(result['time_data'], result['signal_data'], 'r-', linewidth=1.0, label='振动信号')
                    ax_time.set_xlabel('时间 (s)')
                    ax_time.set_ylabel('振动幅值 (g)', color='r')
                    ax_time.tick_params(axis='y', labelcolor='r')
                    
                    # 创建右侧y轴用于显示RMS值
                    ax_rms = ax_time.twinx()
                    rms_value = result['rms_value']
                    
                    # 在时域图上绘制RMS水平线（右侧y轴）
                    ax_rms.axhline(y=rms_value, color='g', linestyle='-', linewidth=2, 
                                label=f'RMS = {rms_value:.4f} g')
                    ax_rms.set_ylabel('RMS (g)', color='g')
                    ...
                    if 'source_file' in result:
                        ...
                        # 频域图标题
                        ax_freq.set_title(f"{channel} {direction} 时间段 {segment_index} (总第{result_idx+1}/{num_segments}段): {result['time_range'][0]:.1f}-{result['time_range'][1]:.1f}秒 - 频域图")
                        # 时域图标题
                        ax_time.set_title(f"{channel} {direction} 时间段 {segment_index} (总第{result_idx+1}/{num_segments}段): {result['time_range'][0]:.1f}-{result['time_range'][1]:.1f}秒 - 时域图（含RMS值）")
                    else:
                        # 单文件结果
                        ax_freq.set_title(f"时间段 {result_idx+1}/{num_segments}: {result['time_range'][0]:.1f}-{result['time_range'][1]:.1f}秒 - 频域图")
                        ax_time.set_title(f"时间段 {result_idx+1}/{num_segments}: {result['time_range'][0]:.1f}-{result['time_range'][1]:.1f}秒 - 时域图（含RMS值）")
                    
                    # 设置频域图属性
                    ax_freq.grid(True, alpha=0.3)
                    ax_freq.set_yscale('log')
                    ax_freq.set_xscale('log')
                    self.set_tick_font(ax_freq)
                    
                    # 设置时域图属性
                    self.set_tick_font(ax_time)
                    self.set_tick_font(ax_rms)
            ...
            if self.current_page >= 0 and self.current_page < self.total_pages:
                ...
                if reply == QMessageBox.Yes:
                    ...
                    for page in range(self.total_pages):
                        # 计算当前页显示的分段范围
                        start_idx = page * segments_per_page
                        end_idx = min(start_idx + segments_per_page, num_segments)
                        ...
                        # 创建临时图形来保存该页面
                        temp_fig = plt.figure(figsize=(10, 10))  # 调整图形高度
                        ...
                        if num_to_show > 0:
                            ...
                            for i in range(num_to_show):
                                ...
                                # 频域图（上方的子图）
                                ax_freq = temp_fig.add_subplot(num_to_show * 2, 1, i * 2 + 1)
                                ax_freq.plot(result['frequency'], result['power_spectrum'], 'b-', linewidth=1.5)
                                ax_freq.set_xlabel('频率 (Hz)')
                                ax_freq.set_ylabel('功率谱密度 (g$^2$/Hz)')
                                
                                # 时域图（下方的子图）- 包含RMS值
                                ax_time = temp_fig.add_subplot(num_to_show * 2, 1, i * 2 + 2)
                            ...
                            # 调整布局
                            temp_fig.subplots_adjust(left=0.1, right=0.9, bottom=0.1, top=0.9, 
                                                hspace=0.6, wspace=0.3)
                            ...
                            if 'total_files' in self.analysis_results:
                                total_files = self.analysis_results['total_files']
                                total_segments = self.analysis_results['total_segments']
                                temp_fig.suptitle(f"多文件功率谱分析结果 - 共{total_files}个文件 {total_segments}个分段 - 第 {page + 1}/{self.total_pages} 页", fontsize=12)
                            else:
                                temp_fig.suptitle(f"功率谱分析结果 - {channel} {direction} 第 {page + 1}/{self.total_pages} 页", fontsize=12)
                        
                        # 保存图片
                        temp_fig.savefig(file_path, dpi=300, bbox_inches='tight')
                        plt.close(temp_fig)
                    
                    QMessageBox.information(self, "保存完成", f"所有页面图片已保存到:\n{save_dir}")
    ...
    def All_Methods_Begin(self):
        ...
        if method == 'MTW':
            ...
            mtw_plot_and_evaluate(self,
                    ...
                    db_within_3_ratio_list = metrics['db_within_3_ratio_list'],
                    total_db_deviation_per_feature = metrics['total_db_deviation_per_feature'],
                    data_test=data_test,  # 修改：传递完整的测试数据
                    y_test=y_test, 
                    y_pred=y_pred
                )
